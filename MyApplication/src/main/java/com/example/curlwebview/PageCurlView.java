...
public class PageCurlView extends WebView {
...
//    private ArrayList<Bitmap> mPages;
    ...
    private WebView foreGroundView;
    private WebView backGroundView;
    private ArrayList<WebView> viewPages;
...
//        mBackground = mPages.get(1);
        ...
        viewPages = new ArrayList<WebView>();

        WebView webView = (WebView) findViewById(R.id.dcgpagecurlPageCurlView);
        webView.setWebViewClient(new CustomWebViewClient());
        webView.loadUrl("http://www.google.co.jp/");
        viewPages.add(webView);
        webView.loadUrl("http://www.yahoo.co.jp/");
        viewPages.add(webView);
        webView.loadUrl("http://www.msn.co.jp/");
        viewPages.add(webView);

        foreGroundView = viewPages.get(0);
        foreGroundView = viewPages.get(1);
...
//        mBackground = temp;
    ...
    private void nextView() {
        ...
        if(foreIndex >= viewPages.size()) {
            foreIndex = 0;
        }
        int backIndex = foreIndex + 1;
        if(backIndex >= viewPages.size()) {
            backIndex = 0;
        }
        mIndex = foreIndex;
    ...
    private void setViews(int foreground, int background) {
        foreGroundView = viewPages.get(foreground);
        backGroundView = viewPages.get(background);
    }
